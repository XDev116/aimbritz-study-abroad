const { useEffect: useEffectNav, useRef: useRefNav, useState: useStateNav } = React;

// Premium nav: slim, "expands" into a drawer with destinations/services on hover/click.
// On scroll: shrinks to a floating capsule pill.
function Nav() {
  const [scrolled, setScrolled] = useStateNav(false);
  const [drawer, setDrawer] = useStateNav(null); // "explore" | "services" | null
  const [menu, setMenu] = useStateNav(false);
  const [time, setTime] = useStateNav("");
  useEffectNav(() => {
  ...
  return (
    <>
      ...
      <header
        ...
        onMouseLeave={() => setDrawer(null)}
      >
        <div
          className={`transition-all duration-500 ${scrolled ? "mx-auto max-w-[1180px] mt-3" : "mx-0 max-w-none mt-0"}`}
        >
          <nav
            className={`flex items-center justify-between transition-all duration-500 ${
              scrolled
                ? "mx-3 rounded-full border px-4 md:px-6 h-12 shadow-[0_12px_40px_-16px_rgba(17,17,19,0.2)]"
                : "px-5 md:px-10 h-16 border-b"
            }`}
            style={{
              background: scrolled ? "rgba(248,244,235,0.92)" : "var(--paper)",
          ...
          >
            ...
            <a href="#" className="flex items-center gap-2.5">
              ...
              >
                <span className="font-serif italic font-black text-[14px] leading-none" style={{ color: "var(--paper)" }}>
                  a
                </span>
              </span>
              <span className="font-sans font-black tracking-[-0.02em] text-[15px]">
                aimbritz<span style={{ color: "var(--ember)" }}>.</span>
              </span>
            </a>
            ...
            <div className="hidden lg:flex items-center gap-1">
              {[
                ...
                >
                  {item.name}
                  <span className="ml-1 text-[8px] opacity-60">{item.key && "↓"}</span>
                  <span className="absolute bottom-0 left-3 right-3 h-px scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500"
            ...
            {drawer === "explore" && (
              <>
                <div className="col-span-4">
                  ...
                  <h3 className="mt-4 font-serif italic text-4xl leading-tight">
                    Where will the next<br/>year take you?
                  </h3>
                </div>
                <div className="col-span-8 grid grid-cols-3 gap-x-8 gap-y-3">
                  {COUNTRIES.map((c) => (
                    <a key={c.code} href="#countries" className="flex items-baseline justify-between border-b pb-2 group" style={{ borderColor: "var(--hairline)" }}>
            ...
            {drawer === "services" && (
              <>
                <div className="col-span-4">
                  ...
                  <h3 className="mt-4 font-serif italic text-4xl leading-tight">
                    Full-stack guidance,<br/>end to end.
                  </h3>
                </div>
                <div className="col-span-8 grid grid-cols-2 gap-x-8 gap-y-4">
                  {[
                    ["01", "Profile Evaluation",   "Match your marks, goals, budget with the right universities."],
                    ["02", "Application Craft",    "SOPs, LORs, essays — refined to admissions standard."],
                    ["03", "Test Prep",            "IELTS · TOEFL · GRE · GMAT · SAT — with our certified coaches."],
                    ["04", "Visa & Travel",        "End-to-end visa filing, mock interviews, flight & housing."],
                    ["05", "Scholarships",         "Merit, need, external — we file on your behalf."],
                    ["06", "Post-Arrival",         "Banking, sim, orientation, and our alumni network on the ground."],
                  ].map(([n, t, d]) => (
                    <a key={n} href="#" className="flex gap-4 border-b pb-4 group" style={{ borderColor: "var(--hairline)" }}>
                      <span className="font-mono text-[10px] text-ember tracking-[0.25em]">{n}</span>
                      <div>
                        <div className="font-sans font-bold uppercase text-[13px] tracking-[0.03em] group-hover:text-ember transition-colors">{t}</div>
                        <div className="text-[12px] text-ink-3 mt-1 leading-relaxed">{d}</div>
                      </div>
                    </a>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden" onClick={() => setMenu(false)}>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[min(420px,100%)] bg-paper p-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-10">
              <span className="font-sans font-black text-lg">aimbritz<span style={{ color: "var(--ember)" }}>.</span></span>
              <button onClick={() => setMenu(false)} className="text-2xl">×</button>
            </div>
            {["Explore", "Services", "Stories", "About", "Contact"].map((n) => (
              <a key={n} href="#" className="block py-3 border-b font-serif italic text-3xl" style={{ borderColor: "var(--hairline)" }}>
                {n}
              </a>
            ))}
            <div className="mt-8 space-y-3">
              <a href="#" className="block text-center py-4 rounded-full t-caps text-[11px]" style={{ background: "var(--ink)", color: "var(--paper)" }}>Book free session →</a>
            </div>
          </div>
        </div>
      )}
