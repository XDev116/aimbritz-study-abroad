...
export function JourneyPinned() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Desktop: CSS sticky + GSAP scrub horizontal scroll (no pin:true)
      mm.add("(min-width: 1024px)", () => {
        const getDistance = () => track.scrollWidth - window.innerWidth + 60;
  ...
  return (
    // Outer sets the scroll distance; inner is sticky
    <div ref={outerRef} className="relative lg:h-[500vh]" style={{ background: "transparent" }}>
      <div className="lg:sticky lg:top-0 lg:h-screen lg:overflow-hidden">
        {/* Mobile: vertical stack; Desktop: horizontal track */}
        <div
          ref={trackRef}
          className="flex flex-col lg:flex-row lg:w-max lg:h-screen will-change-transform"
        >
          {JOURNEY.map((p, i) => (
            <div
              key={p.n}
              className="relative lg:w-screen lg:flex-shrink-0 lg:h-screen flex items-center"
              style={{
            ...
            >
              ...
              <div className="relative z-10 px-10 lg:px-20 max-w-2xl">
                ...
                <div
                  ...
                  style={{ borderColor: "var(--ember)", background: "rgba(194,65,12,0.08)" }}
                >
